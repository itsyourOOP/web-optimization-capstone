...
class Agent:
  ...
  def run(self): 
    ...
    # perf monitor thread
    perf_mon = PerfMonitor(self._pid) 
    perf_t = Thread(target=perf_mon.run, args=()) 
    perf_t.start() 
  
    # power monitor thread
    power_mon = PowerMonitor() 
    power_t = Thread(target=power_mon.run, args=()) 
    power_t.start() 

    # core power management
    core_man = CoreManagement(self._pid)

    core_man.set_little_governor("userspace")
    core_man.set_big_governor("userspace")

    # initial configuration
    cur_config = (2, 4, 1200000, 1200000)
    core_man.set_config(cur_config[0], cur_config[1], cur_config[2], cur_config[3])
    ...
    while self._running: 
      ...
      if perf_sample is None:
        sleep(0.1)
        perf_sample = perf_mon.last_sample()
      self._observations.append((perf_sample, cur_config))
      ...
      rand_mapping = choice(core_mappings)
      rand_big_freq = choice(big_freqs)
      rand_little_freq = choice(little_freqs)

      # perform action
      action = (rand_mapping[0], rand_mapping[1], rand_big_freq, rand_little_freq)
      core_man.set_config(action[0], action[1], action[2], action[3])
      ...
      if power_sample is None:
        time(0.1)
        power_sample = power_mon.last_sample()
      self._rewards += [-power_sample[1]]

    ts2 = time()

    # final reward
    self._rewards += [(power_mon.report_energy(), (ts2-ts1))]
...
pages_old = {'amazon':'http://tucunare.cs.pitt.edu:8080/amazon/www.amazon.com/index.html',
...
sleep(5)
browser.get("http://www.cs.pitt.edu")
sleep(5)

pid = int(os.popen("pidof chromium-browser").read())
...
while agent_running:
  ...
  # select random page
  p = choice(list(pages.keys()))

  agent = Agent(pid)
  agent_t = Thread(target=agent.run, args=()) 
  agent_t.start() 
...
#with open('./baseline-results/page-load-times.txt', 'w') as fp:
